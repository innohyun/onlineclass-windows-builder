...
pub(crate) const SERVICE_VERSION: &str = "2026-09-17.1-sync-artifact-recovery";
...
const LOCAL_SENSITIVE_STORE_ROUTES: &[&str] = &[
    ...
    "/v1/classaimate-mcp/life-records/list",
...
fn handle_request(
    ...
    let result = (|| -> Result<(u16, Value), String> {
        ...
        if path.starts_with("/v1/classaimate-mcp/") {
            ...
            if request.method() == &Method::Post && path == "/v1/classaimate-mcp/life-records/list" {
                let body = scope_body_to_tenant(read_body(&mut request)?, Some(&tenant))?;
                return Ok((200, classaimate_mcp_life_records::list(&store, &body)?));
            }
...
#[tauri::command]
fn acknowledge_desktop_activation_intent(
    intent: desktop_activation::DesktopActivationIntent,
    state: tauri::State<'_, desktop_activation::DesktopActivationState>,
) -> bool {
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            acknowledge_desktop_activation_intent,
